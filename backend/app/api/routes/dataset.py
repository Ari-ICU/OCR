...
from app.core.security import sanitize_filename, is_safe_url, validate_file_signature
...
async def convert_url_pdf_to_txt(req: UrlConvertToTxtRequest):
    ...
    Fetches the PDF directly on the API server from the provided URL (e.g. server store or cloud link),
    optionally caches it into ./pdf/, converts it to clean text & LaTeX on the server,
    ...
    # Save to server ./pdf/ folder if requested
